...
class SamplerSASolverExperimental:
    ...
    def get_sampler(self, model, vae, pc_mode, invert, normalize, smea, dyn, renoise, renoise_alternative, eta, eta_start_percent, eta_end_percent, scale, shift, gamma):
        model_sampling = model.get_model_object('model_sampling')
        start_sigma = model_sampling.percent_to_sigma(eta_start_percent)
        end_sigma = model_sampling.percent_to_sigma(eta_end_percent)
        tau_func = partial(sa_solver.default_tau_func, eta=eta, eta_start_sigma=start_sigma, eta_end_sigma=end_sigma)
        sampler = comfy.samplers.ksampler("sa_solver_experimental", {"tau_func": tau_func, "vae": vae, "pc_mode": pc_mode, "invert": invert, "normalize": normalize, "smea": smea, "dyn": dyn, "scale": scale, "shift": shift, "gamma": gamma, "renoise": renoise, "renoise_alternative": renoise_alternative})
        return (sampler, )